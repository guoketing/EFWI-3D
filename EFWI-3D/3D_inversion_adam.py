...
result_dir_name = './' + exp_name + '/Results_Adam'
figure_dir_name = './' + exp_name + '/Figures_Adam'


criterion = torch.nn.MSELoss()
# optimizer = torch.optim.Adam([{'params': [fwi.Vp], 'lr': 20}, {'params': [fwi.Vs], 'lr': 10}, {'params': [fwi.Rho], 'lr': 10}])
optimizer = torch.optim.Adam([{'params': [fwi.Vp], 'lr': 20}])


for epoch in range(nIter):
    optimizer.zero_grad()
    comLoss = fwi(Shot_ids, ngpu=ngpu)
    ...
    if epoch % 1 == 0:
        ...
        plt.title("rho")
        plt.savefig(figure_dir_name + "/Rho"  + str(epoch) + '.png')
