...
Parameter::Parameter(const std::string &para_fname, int calc_id) {
  ...
  assert(json_para.HasMember("ny"));
  assert(json_para["ny"].IsInt());
  ny_ = json_para["ny"].GetInt();
#ifdef VERBOSE
  cout << "	ny = " << ny_ << endl;
#endif
  ...
  assert(json_para.HasMember("dy"));
  assert(json_para["dy"].IsNumber());
  dy_ = json_para["dy"].GetDouble();
#ifdef VERBOSE
  cout << "	dy = " << dy_ << endl;
#endif
