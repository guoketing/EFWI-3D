...
Src_Rec::Src_Rec(Parameter &para, string survey_fname, const float *stf, int group_size, const int *shot_ids) {
	...
	for (int i = 0; i < group_size; i++) {
		...
		cudaHostAlloc((void**)&h_data_x, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
		initialArray(h_data_x, nSteps*nrec, 0.0);
		vec_data_x.push_back(h_data_x);

		cudaHostAlloc((void**)&h_data_y, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
		initialArray(h_data_y, nSteps*nrec, 0.0);
		vec_data_y.push_back(h_data_y);

		cudaHostAlloc((void**)&h_data_z, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
		initialArray(h_data_z, nSteps*nrec, 0.0);
		vec_data_z.push_back(h_data_z);

		if (para.if_res()) {
			// initialize the host side observed data cube
			cudaHostAlloc((void**)&h_data_obs_x, nSteps*nrec*sizeof(float), cudaHostAllocDefault);
			initialArray(h_data_obs_x, nSteps*nrec, 0.0);
			vec_data_obs_x.push_back(h_data_obs_x);
			// initialize the host side data residual
			cudaHostAlloc((void**)&h_res_x, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
			initialArray(h_res_x, nSteps*nrec, 0.0);
			vec_res_x.push_back(h_res_x);

			// initialize the host side observed data cube
			cudaHostAlloc((void**)&h_data_obs_y, nSteps*nrec*sizeof(float), cudaHostAllocDefault);
			initialArray(h_data_obs_y, nSteps*nrec, 0.0);
			vec_data_obs_y.push_back(h_data_obs_y);
			// initialize the host side data residual
			cudaHostAlloc((void**)&h_res_y, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
			initialArray(h_res_y, nSteps*nrec, 0.0);
			vec_res_y.push_back(h_res_y);

			// initialize the host side observed data cube
			cudaHostAlloc((void**)&h_data_obs_z, nSteps*nrec*sizeof(float), cudaHostAllocDefault);
			initialArray(h_data_obs_z, nSteps*nrec, 0.0);
			vec_data_obs_z.push_back(h_data_obs_z);
			// initialize the host side data residual
			cudaHostAlloc((void**)&h_res_z, nSteps*nrec*sizeof(float), cudaHostAllocDefault); //test
...
Src_Rec::~Src_Rec() {
	...
	for(int i=0; i<vec_data_x.size(); i++) {
		CHECK(cudaFreeHost(vec_data_x.at(i))); //test
		CHECK(cudaFreeHost(vec_data_y.at(i))); //test
		CHECK(cudaFreeHost(vec_data_z.at(i))); //test
	}

	if (if_res_) {
		for(int i=0; i<vec_data_obs_x.size(); i++) {
			CHECK(cudaFreeHost(vec_data_obs_x.at(i)));
			CHECK(cudaFreeHost(vec_data_obs_y.at(i)));
			CHECK(cudaFreeHost(vec_data_obs_z.at(i)));
			CHECK(cudaFreeHost(vec_res_x.at(i)));
			CHECK(cudaFreeHost(vec_res_y.at(i)));
			CHECK(cudaFreeHost(vec_res_z.at(i)));
		}
	}
